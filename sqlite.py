...
class sqlite :
    ...
    def crearTabla (self, nombreTabla, campos) :
        ...
        campos, es un diccionario con formato clave='nombreCampo'; 
        valor='tipo sqlite' """
    ...
    def crearVista(self, nombreVista, contenidoVista):
        ...
        if self.realizarNoConsulta(SQL_CREATE_VISTA):         
            print 'Vista %s creada...' %nombreTabla
            return True
        else:
    ...
    def getColumnasTabla(self, nombreTabla) :
        """ Obtiene los nombres de las columnas en la BD. """
        columnas = []
    ...
    def getDatosColumna (self, tabla, columna, loquebusco) :
        """ Recibe un patron y una columna y devuelve las coincidencias de dicho patron en la tabla. """
    ...
    def getDatosPatronColumnasEspecificas(self, tabla, patron, columna, listacolumnas):
        ...
        listamatch, lista,items = self.getDatosPatronEnColumna(tabla, patron, columna), [],[]
    ...
    def getTiposCampos(self, nombreTabla):
        """ Obtiene los tipos de los campos en la tabla. """
        lista = []
        self.__cursor.execute('PRAGMA table_info(' + nombreTabla + ')')
        infotable = self.__cursor.fetchall()
        print infotable
    ...
    def realizarModificacion (self, tabla, sets, condiciones) :
        """ Realiza un UPDATE en la tabla y campos indicados, que cumplan con las condiciones especificadas. 
        tabla = string,
        sets = dict,
        condiciones = lista o tupla """ 
...
def main():
    ...
    p = sqlite('probando.db')    
    #~ p.realizarModificacion('fruta',{'nro':10,'nombre': "'juan'"},('nro = 10','nombre = "juan"'))
    #{'nro':10,'nombre': 'juan'}
    #~ p.crearTabla("alumnos",{
    #~ "id_alumno":"int not null",
    #~ "PK":"id_alumno",
    #~ "nombre_alumno":"varchar(20) not null",
    #~ "ap_alumno":"varchar(20)"})
    
    #~ p.realizarAlta("alumnos",{"id_alumno":1,"nombre_alumno":"Jonathan","ap_alumno":"Ferreyra"})
    #~ p.realizarAlta("alumnos",{"id_alumno":2,"nombre_alumno":"Alejandro","ap_alumno":"Ferreyra"})
    #~ p.realizarAlta("alumnos",{"id_alumno":3,"nombre_alumno":"Ayelen","ap_alumno":"Ferreyra"})
    
    #~ p.realizarModificacion("alumnos",{"id_alumno":4},("id_alumno = 1",))
    
    #~ p.realizarBaja("alumnos",["id_alumno = 1"])
    
    #~ print p.realizarConsulta("select * from alumnos")
    #~ print p.getTiposCampos("alumnos")
